...
PostgreSQL に格納。AI分析やUI表示の基礎データとして活用します。

Automatically fetches daily and previous-day economic indicators from Investing.com
and stores them in PostgreSQL for AI analysis and dashboard visualization.

⸻

⚙️ 主な処理フロー / Processing Flow
	1.	Playwrightでサイトへアクセス / Access Investing.com
	•	「Today」または「Yesterday」タブをクリックしてデータ取得
	2.	各指標情報の抽出 / Extract Indicator Data
	•	国コード（Country）
	•	指標名（Indicator Name）
	•	結果・予想・前回値（Actual / Forecast / Previous）
	•	発表ステータス（結果あり・未発表）
	•	重要度（LOW / MEDIUM / HIGH）
	3.	PostgreSQLへUpsert（既存データの自動更新）
...
DO UPDATE SET
  ...
  status = CASE
    WHEN EXCLUDED.actual_value IS NOT NULL THEN '結果あり'
    ELSE '未発表'
  END,
...
前回値 / Previous value
status
発表状況（結果あり・未発表） / Publication status
importance
重要度（HIGH / MEDIUM / LOW） / Importance level
last_updated
最終更新日時 / Last updated timestamp

🕐 実行方法 / How to Run
python fetch_economic_calendar_today_with_status_and_importance.py
...
AI分析も最新の経済状況を基に実行されるようになります。

⸻

🧠 今後の展望 / Future Vision
	•	経済指標 × 市場反応のAI分析連携
→ Correlation analysis between indicators and market movements
	•	政策金利・要人発言データの追加
→ Add central bank rates & statements
	•	自動取引エンジンとの統合
